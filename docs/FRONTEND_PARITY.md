...
| Workflows | One on-disk format, read through `jterm_core::workflows` from the same per-app directories in the same precedence order, under the same per-directory caps. A file one frontend rejects the other must reject: same validation (including an argument `name` equal to its own trim), same bounded reader with `O_NOFOLLOW`, same `{name}`/`{{name}}` template rules including literal-brace escapes and an unmatched `{{` preserved as authored. Same required-argument contract: an argument with no declared `default` must be supplied or the render is refused with `missing values: <names>`, while `default = ""` is an explicit empty value. The bundled example libraries must stay byte-identical — dedup is name-keyed and first-wins, so a divergent copy of a shared name silently resolves to a different command per frontend. |
