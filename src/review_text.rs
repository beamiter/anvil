//! Anvil-specific review-text helpers layered on the shared
//! `jterm_core::review_input` boundary.
//!
//! The visual-spoofing character class itself lives in core; this module keeps
//! only anvil's per-surface byte budgets, the parameterized strict validator,
...
///
/// LF/tab retain anvil's structural paste semantics and CR/CRLF normalize to
...
mod tests {
    ...
    #[test]
    fn strict_validator_delegates_spoofing_to_core_and_keeps_anvil_budgets() {
        // The full character class is covered by jterm_core::review_input;
        // here only the wiring and anvil's parameterized budget are checked.
