...
# Doctor output is collected only with its privacy mode enabled. Configuration
# validation still reports its input path, so only its exit status is retained.
doctor_status=0
...

{
    ...
    if [[ -d "${config_home}/jterm1" ]]; then
        ...
        printf 'session snapshots: missing\n'
    fi
...
The diagnostic statuses may still reveal whether configured features are ready.
